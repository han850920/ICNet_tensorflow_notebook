...
def main():
    ...
    indices = tf.squeeze(tf.compat.v1.where(mask), 1)
    ...
    if cfg.dataset == 'ade20k':
        ...
        mIoU, update_op = tf.compat.v1.metrics.mean_iou(predictions=pred, labels=gt, num_classes=cfg.param['num_classes']+1)
    elif cfg.dataset == 'cityscapes':
        mIoU, update_op = tf.compat.v1.metrics.mean_iou(predictions=pred, labels=gt, num_classes=cfg.param['num_classes'])
