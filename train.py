...
def create_losses(net, label, cfg):
    ...
    l2_losses = [cfg.WEIGHT_DECAY * tf.nn.l2_loss(v) for v in tf.compat.v1.trainable_variables() if 'weights' in v.name]
...
def main():
    ...
    restore_var = [v for v in tf.compat.v1.global_variables() if 'conv6_cls' not in v.name] # add this line to train (mintur)

    all_trainable = [v for v in tf.compat.v1.trainable_variables() if ('beta' not in v.name and 'gamma' not in v.name) or args.train_beta_gamma]
    ...
    with tf.control_dependencies(update_ops):
        opt_conv = tf.compat.v1.train.MomentumOptimizer(learning_rate, cfg.MOMENTUM)
