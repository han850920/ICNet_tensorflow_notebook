...
DEFAULT_DATAFORMAT = 'channels_first'
layer_name = []
...
class Network(object):
            ...
            with tf.compat.v1.variable_scope(op_name, reuse=True):
                for param_name, data in data_dict[op_name].items():
                    try:
                        ...
                        if 'conv6_cls' not in var.name: ## add this line to train from scratch (mintur)
    ...
    def conv(self,
             ...
             group=1,
             biased=True):
        print(k_h,k_w,c_o,s_h,s_w,name)
        ...
        convolve = lambda i, k: tf.nn.conv2d(input=i, filters=k, strides=[1, s_h, s_w, 1], padding=padding,data_format=DEFAULT_DATAFORMAT)
        #should be tf.keras.layers.Conv2D(filters=c_o,kernel_size=[k_h, k_w, c_i, c_o],strides=[1, s_h, s_w, 1],padding=padding,data_format=DEFAULT_DATAFORMAT)
    ...
    def add(self, inputs, name):
        inputs[0] = tf.image.resize_bilinear(inputs[0], size=tf.shape(input=inputs[1])[1:3],method=tf.image.ResizeMethod.BILINEAR)
    ...
    def resize_bilinear(self, input, size, name):
        return tf.image.resize(input, size=size, method=tf.image.ResizeMethod.BILINEAR, antialias=True, name=name)
    ...
    def interp(self, input, s_factor=1, z_factor=1, name=None):
        ...
        return tf.image.resize(input, size=resize_shape, method=tf.image.ResizeMethod.BILINEAR, antialias=True, name=name)
