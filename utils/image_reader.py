...
def _image_scaling(img, label):
    ...
    label = tf.image.resize_nearest_neighbor(tf.expand_dims(label, 0), new_shape,method=ResizeMethod.NEAREST_NEIGHBOR)
...
class ImageReader(object):
    ...
    def __init__(self, cfg, img_path=None, mode='eval'):
        if mode == 'train' or mode == 'eval':
            ...
            self.next_image, self.next_label = tf.compat.v1.data.make_one_shot_iterator(self.dataset).get_next() 
