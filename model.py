...
class ICNet_BN(Network):
    def __init__(self, cfg, mode, image_reader=None):
        ...
        if mode == 'train':
            ...
            self.img_placeholder = tf.compat.v1.placeholder(dtype=tf.float32, shape=cfg.INFER_SIZE)
    ...
    def get_output_node(self):
        if self.mode == 'inference':
            ...
            logits_up = tf.image.resize(logits, size=self.n_shape, method=tf.image.ResizeMethod.BILINEAR)
            logits_up = tf.image.crop_to_bounding_box(logits_up, 0, 0, self.o_shape[0], self.o_shape[1])

            output_classes = tf.argmax(input=logits_up, axis=3)
            output = decode_labels(output_classes, self.o_shape, self.cfg.param['num_classes'])

        elif self.mode == 'eval':
            logits = self.layers['conv6_cls']

            logits_up = tf.image.resize(logits, size=tf.shape(input=self.labels)[1:3], method=tf.image.ResizeMethod.BILINEAR)
            output = tf.argmax(input=logits_up, axis=3)
    ...
    def setup(self):
        ...
        (self.feed('conv4_1_1x1_proj_bn',
             ...
             .zero_padding(paddings=2, name='2')
